...
pub fn get_eval_cases(conn: &Connection, run_id: &str) -> Result<Vec<CaseResult>, String> {
    ...
    let rows = stmt
        .query_map(params![run_id], |row| {
            ...
            let checks: Vec<super::scoring::CheckResult> =
...
pub fn get_eval_goals(
    ...
    if let Some(s) = skill {
        ...
        for row in rows {
            results.push(row.map_err(|e| format!("Failed to read: {}", e))?);
        }
    } else {
...
pub fn update_eval_goal_progress(
    conn: &Connection,
    goal_id: i64,
    run_id: &str,
    metric_value: f64,
    ...
    conn.execute(
        "UPDATE eval_goal SET current_value = ?1, updated_at = datetime('now') WHERE id = ?2",
        params![metric_value, goal_id],
    )
...
pub fn get_eval_suggestions(
    ...
    let make_json = |row: &rusqlite::Row| -> rusqlite::Result<serde_json::Value> {
        Ok(
            serde_json::json!({"id": row.get::<_, i64>(0)?, "skill": row.get::<_, String>(1)?, "suggestion_type": row.get::<_, String>(2)?, "priority": row.get::<_, String>(3)?, "title": row.get::<_, String>(4)?, "status": row.get::<_, String>(7)?}),
        )
    };
    match (skill, status) {
        (Some(s), Some(st)) => {
            ...
            for row in stmt
                .query_map(params![s, st], make_json)
...
pub fn save_context_case(
    conn: &rusqlite::Connection,
    case_id: &str,
    skill: &str,
    context: &super::case::EvalContext,
) -> Result<i64, String> {
...
pub fn get_eval_baseline(
    ...
    let result = stmt
        .query_row(params![skill, case_id], |row| {
            Ok(serde_json::json!({
                "id": row.get::<_, i64>(0)?,
        ...
        .optional()
        .map_err(|e| format!("Failed to query baseline: {}", e))?;

    Ok(result)
}

pub fn get_eval_baselines(
    conn: &Connection,
    skill: &str,
) -> Result<Vec<serde_json::Value>, String> {
    let mut results = Vec::new();
...
pub fn get_promoted_status(conn: &Connection, skill: &str) -> Result<bool, String> {
    let mut stmt = conn
        .prepare("SELECT COUNT(*) FROM eval_promoted WHERE skill = ?1")
        .map_err(|e| format!("Failed to prepare: {}", e))?;

    let count: i64 = stmt
        .query_row(params![skill], |row| row.get(0))
        .map_err(|e| format!("Failed to query: {}", e))?;
...
pub fn set_promoted_status(conn: &Connection, skill: &str, promoted: bool) -> Result<(), String> {
    if promoted {
        ...
        conn.execute("DELETE FROM eval_promoted WHERE skill = ?1", params![skill])
    ...
    match (run_id, case_id) {
        (Some(r), Some(c)) => {
            let mut stmt = conn.prepare("SELECT id, run_id, case_id, prompt_score, response_score, context_score, overall_score, feedback, created_at FROM eval_quality_score WHERE run_id = ?1 AND case_id = ?2 ORDER BY created_at DESC")
                .map_err(|e| format!("Failed to prepare: {}", e))?;
            let rows = stmt
                .query_map(rusqlite::params![r, c], |row| {
                    Ok(serde_json::json!({
                        "id": row.get::<_, i64>(0)?,
                        "run_id": row.get::<_, String>(1)?,
                        "case_id": row.get::<_, String>(2)?,
        ...
        (Some(r), None) => {
            let mut stmt = conn.prepare("SELECT id, run_id, case_id, prompt_score, response_score, context_score, overall_score, feedback, created_at FROM eval_quality_score WHERE run_id = ?1 ORDER BY created_at DESC")
                .map_err(|e| format!("Failed to prepare: {}", e))?;
            let rows = stmt
                .query_map(rusqlite::params![r], |row| {
                    Ok(serde_json::json!({
                        "id": row.get::<_, i64>(0)?,
                        "run_id": row.get::<_, String>(1)?,
                        "case_id": row.get::<_, String>(2)?,
