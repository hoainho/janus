...
mod tests {
    ...
    fn test_attribution_cross_skill_change() {
        ...
        assert_eq!(
            attribution.suspected_skills.as_ref().unwrap(),
            &["other-skill".to_string()]
        );
    }
    ...
    fn test_storage_create_and_get_goal() {
        ...
        assert_eq!(
            goals[0].get("goal_type").unwrap().as_str().unwrap(),
            "regression_detection"
        );
    }
    ...
    fn test_storage_update_goal_progress() {
        ...
        crate::eval::storage::update_eval_goal_progress(
            &conn,
            goal_id,
            "run-001",
            92.5,
        ...
        assert_eq!(
            progress[0].get("metric_value").unwrap().as_f64().unwrap(),
            92.5
        );
    }
    ...
    fn test_storage_create_and_get_suggestion() {
        ...
        let suggestions =
            crate::eval::storage::get_eval_suggestions(&conn, Some("test-skill"), Some("pending"))
                .unwrap();
        assert_eq!(suggestions.len(), 1);
        assert_eq!(
            suggestions[0].get("title").unwrap().as_str().unwrap(),
            "Fix flaky test"
        );
    }
    ...
    fn test_context_relevance_scoring() {
        ...
        let relevant_score =
            crate::eval::quality::score_context_relevance(response, relevant_context);
        let irrelevant_score =
            crate::eval::quality::score_context_relevance(response, irrelevant_context);
