...
pub fn score_prompt_quality(prompt: &str) -> f64 {
    ...
    let has_specificity = prompt.contains("specific") || prompt.contains("exact") || prompt.contains("particular");
    
    let mut score: f64 = 0.0;
    
    if word_count >= 10 { score += 25.0; }
    if word_count >= 25 { score += 15.0; }
    if has_question { score += 20.0; }
    if has_context { score += 25.0; }
    if has_specificity { score += 15.0; }
...
pub fn score_response_quality(response: &str) -> f64 {
    ...
    let has_conclusion = response.contains("summary") || response.contains("conclusion") || response.contains("in summary");
    
    let mut score: f64 = 0.0;
    
    if word_count >= 50 { score += 20.0; }
    if word_count >= 100 { score += 15.0; }
    if word_count >= 200 { score += 10.0; }
    if has_code { score += 15.0; }
    if has_examples { score += 15.0; }
    if has_structure { score += 15.0; }
    if has_conclusion { score += 10.0; }
